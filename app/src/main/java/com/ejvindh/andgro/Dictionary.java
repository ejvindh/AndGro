...
public class Dictionary {
	...
	static String[][] lookup(String search_string_orig,
							 String gddfile, String datfile, String dict_dir,
							 int doubflag) throws IOException {
		...
		for (int x=0; x<=((doubflag^1)&2)/2; x++) { //ligningen giver 1, hvis envejs; ellers 2
			if (results[x][0].equals("")) {
				StringBuilder henv = new StringBuilder();
	...
	private static ArrayList<int[]> getEntries(SQLiteDatabase sql_conn, String search_string, String table, boolean d) {
		...
		ArrayList<String> src_terms_list = new ArrayList<>(Arrays.asList(src_terms));
        int fromDanish;
        if (d) {
			fromDanish=1;
		} else fromDanish=2;
        boolean first_term = true;
		ArrayList<Integer> entry_ids = new ArrayList<>();
		ArrayList<Integer> term_entry_ids = new ArrayList<>();
		for (String term : src_terms_list) {
			...
			if (first_term) {
				...
				ArrayList<Integer> a1 = new ArrayList<>(entry_ids);
	    		entry_ids.clear();
		...
		for (int entry_id: entry_ids) {
			...
			@SuppressLint("Recycle") Cursor rows = sql_conn.rawQuery(selectQuery, null);
			if (rows.moveToFirst()) {
	            do {
	...
	private static String getRawEntryText(RandomAccessFile dat_file, ArrayList<int[]> entries) throws IOException {
		ArrayList<String> raw_entries = new ArrayList<>();
		...
		StringBuilder raw_entries_collect_to_string = new StringBuilder();
